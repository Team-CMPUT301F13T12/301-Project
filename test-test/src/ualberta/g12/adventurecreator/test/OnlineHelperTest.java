...
public class OnlineHelperTest extends
    ...
    /**
     * Tests inserting a story into the web server by seeing if the size of the
     * collection retrieved is bigger than zero and also checking that the
     * object inside is not null
     */
     ...
     * id, title and also author are the same
     * 
     * @throws IOException
    ...
    /**
     * Tests getAllStoryTitlesIdAuthor method we add a new story into the web
     * sever for each story we get back then we check if title and author are
     * not null so they have something then we check if the Id is not -1 because
     * only invalid ids will be this value then we check if the fragment size is
     * 1 because all fragments will have a size of 1(starting fragment) if none
     * has been added to it Test does this and that
     * 
     * @throws IOException
     * @throws IllegalStateException
     */
    public void testGetAllStoryTiltesIdAuthor() throws IllegalStateException, IOException {
        ...
        for (int i = 0; i < myStories.size(); i++) {
            ...
            assertTrue(myStories.get(i).getFragments().size() >= 0);
        }

    }

    /**
     * Test if the checkID method is working for the online helper. It will be
     * tested by adding a story with an id we know and seeing if the result of
     * checkId is true with the inserted stories id
     * 
     * @throws IOException
     * @throws IllegalStateException
     */
    ...
    /**
     * Tests the search search method which is used to search the web server for
     * a particular story This is tested by adding a story in which we know the
     * title and author we then "search" for part of the title and we find if
     * the returned stories contains the inserted story
     * 
     * @throws IOException
     * @throws IllegalStateException
     */
     ...
     * We first add a story and get the id we then get the size of available
     * stories after we call the deleteStories method and to make sure we
     * deleted the item we get the size of the library again and make sure it
     * decremented by 1
     * 
     * @throws IllegalStateException
     * @throws IOException
