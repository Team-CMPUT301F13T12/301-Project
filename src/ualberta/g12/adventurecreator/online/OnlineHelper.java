...
/**
 * This is the class in which all our backend web based service functions are located. 
 * OnlineHelper provides interfaces with the web server to obtain, edit and retrieve information
 * from. This class is based off of code found here:https://github.com/rayzhangcl/ESDemo
 * @author Vincent
 *
 */
public class OnlineHelper {
	...
	/**
	 * insertStory is used to insert a story into our webserver. However, it is also used
	 * to update a story because of how the webserver works.
	 * The story inserted should have a unique id before being inserted.
	 * As our webserver only allows one instance of an object with certain type and id 
	 * inserting an object with an already existing id will overwrite it.
	 * @param story
	...
	/**
	 * Gets the story by the id from our server. An Id should be unique to that story object.
	 * All ids are generated by looking at the title and author of a story. 
	 * If an id is not in the server then the resulting Story object should be NULL.
	 * 
	...
	/**
	 * This method is used to get ALL stories in our web server.
	 * Warning: this can be very slow if stories contain many pictures and media, or if there
	 ...
	 * associated with it
	 * 
	 * @return an ArraList of partial stories with only the author, title and id fields filled in
	...
	/**
	 * This method searches our web server for the input string on fields author and story.
	 * We used the input string "search" to in an elastic search query for fields storyTitle and 
	 * also author to find hits that match the "search" string.
	 * 
	 * Method should be used for some search functionality in our main program
	...
	/**
	 * deleteStories deletes ALL stories with the same story ID.
	 * 
	 * If there were a case where there was more than one story with the same ID (which should not happen)
	 * ALL stories with the same ID will be deleted from our web server. 
